...
const CreateCourse = () => {
	...
	const { handleAddCourse } = useCoursesContext();
	...
	const handleOnAddAuthor = (author: Author) => {
		if (!author.isCourseAuthor) {
			// Only proceed if the author is not already a course author
			handleDeleteAuthor(author.id); // Mark as not a course author in AuthorsProvider
	...
	const handleOnDeleteAuthor = (Author: Author) => {
		if (Author.isCourseAuthor) {
			// Only proceed if the author is a course author
			handleAddAuthor(Author.name); // Add back to all authors in AuthorsProvider
			setCourseAuthors((prev) =>
